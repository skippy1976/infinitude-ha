/*
    ...
    Note that this is an extension to the Hubitat Virtual Thirmistat
    https://github.com/hubitat/HubitatPublic/blob/master/examples/drivers/virtualThermostat.groovy

* Intial release: 0.0.1
*/

def version() { return "Infinitude Zone Driver 0.0.1" }
...
def setRelativeHumidity(humidity) {
	logDebug "setRelativeHumidity(${humidity}) was called"    
    sendEvent(name: "humidity", value: humidity, unit: "%", descriptionText: getDescriptionText("relative humidity set to ${humidity}%"))
}

def setHumiditySetpoint(setpoint) {
	logDebug "setHumiditySetpoint(${setpoint}) was called"
	sendEvent(name: "humiditySetpoint", value: setpoint, unit: "%", descriptionText: getDescriptionText("humidity setpoint set to ${setpoint}%"))
}
...
def setThermostatMode(mode) {
    logDebug "setThermostatMode(${mode}) was called"
...
private updateSetpoints(sp = null, hsp = null, csp = null, operatingState = null){
    logDebug "updateSetpoints(${sp},${hsp},${csp},${operatingState})"
    ...
    logDebug "state changes ${spChange} ${hspChange} ${cspChange} ${osChange}"
    ...
    logDebug "last operating mode set to ${newOS}"
